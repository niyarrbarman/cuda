...
int main(){
    ...
    // cublas needs a handle — basically a context for all blas operations
    cublasHandle_t handle;
    CHECK_CUBLAS(cublasCreate(&handle));

    // ========== SGEMM (single precision, float32) ==========
    // allocate device memory for float32 version
    float *d_A, *d_B, *d_C;
    ...
    // ========== HGEMM (half precision, float16) ==========
    // same thing but with fp16 — uses tensor cores on newer gpus
    ...
    // convert float32 inputs to float16 on host before copying
    // __float2half is a cuda intrinsic for this conversion
    ...
    // hgemm — same pattern as sgemm, just different types
